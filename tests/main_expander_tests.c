...
static t_token	*mock_token_new(t_token_type type, const char *value, int sq,
                               int dq) {
  t_token *tok;
...
static void	test_expand_one_token(void) {
  ...
  test_result("expand_one_token NULL token",
              expand_one_token(NULL, &shell) == 1);

  tok = mock_token_new(TOK_PIPE, "|", 0, 0);
  test_result("expand_one_token non-word success",
              expand_one_token(tok, &shell) == 1);
